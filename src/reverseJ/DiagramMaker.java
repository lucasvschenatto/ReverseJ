...
//		outputDir = new File().getCanonicalFile();
		URI outputURI = URI.createFileURI("../ReverseJ/files/ExtendedPO2")
//		.appendSegment("files")
//		.appendSegment("ExtendedPO2")
		.appendFileExtension(UMLResource.FILE_EXTENSION);
		save(myModel, outputURI);
	} 
	...
	public static void oldMain(String[] args) throws Exception {
		...
		createAssociation(purchaseOrderClass, true,
				AggregationKind.COMPOSITE_LITERAL, "shipTo", 1, 1,
				addressClass, false, AggregationKind.NONE_LITERAL, "", 1, 1);
	...
	protected static Association createAssociation(Type type1,
		...
		out("Association %s [%s..%s] %s-%s %s [%s..%s] created.", //
				...
				? String.format("{%s}", type1.getQualifiedName()) //
						// user-specified name
						: String.format("'%s::%s'", type1.getQualifiedName(),
								end1Name), //
				end1LowerBound, // no special case for this
				(end1UpperBound == LiteralUnlimitedNatural.UNLIMITED) ? "*" // special
																			// case
																			// for
																			// unlimited
																			// upper
				...
				? String.format("{%s}", type2.getQualifiedName()) //
						// user-specified name
						: String.format("'%s::%s'", type2.getQualifiedName(),
								end2Name), //
				end2LowerBound, // no special case for this
				(end2UpperBound == LiteralUnlimitedNatural.UNLIMITED) ? "*" // special
																			// case
																			// for
																			// unlimited
																			// upper
	...
	private static boolean processArgs(String[] args) throws IOException {
		...
		if (args.length != 1) {
			...
			err("Usage: java -jar ... %s <dir>",
					DiagramMaker.class.getSimpleName());
			err("where");
